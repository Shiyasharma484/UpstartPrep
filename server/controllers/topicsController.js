...
exports.createtopics = async (req, res) => {
  ...
  try {
    ...
    try {
      const create_data = await SQL.addData("topics", data);
...
exports.gettopicsByID = async (req, res) => {
  ...
  if (req.params.id && req.params.id.length > 0) {
    ID = req.params.id;
    try {
      const get_topics = await SQL.query(
        "SELECT t.id,t.topic_name, t.skill_id, sk.skill_name,t.short_description,t.active FROM topics as t LEFT JOIN skills as sk ON t.skill_id= sk.id WHERE t.id=" +
          ID
      );
      g_response["status"] = "success";
      g_response["responsedata"] = get_topics;
      g_status_code = 200;
    } catch (err) {
      g_response["status"] = "error";
      g_response["message"] = err.message;
...
exports.getAlltopics = async (req, res) => {
  ...
  try {
    const get_topics = await SQL.query(
      "SELECT t.id,t.topic_name, t.skill_id, sk.skill_name,t.short_description,t.active FROM topics as t LEFT JOIN skills as sk ON t.skill_id= sk.id"
    );
    g_response["status"] = "success";
    g_response["responsedata"] = get_topics;
    g_status_code = 200;
...
exports.updatetopicsByID = async (req, res) => {
  ...
  if (req.params.id && req.params.id.length > 0) {
    ...
    try {
      const valid_data = await validation(data, validationRule);
      const update_topics_data = await SQL.Updatedata("topics", data, {
        id: ID,
      });

      g_response["status"] = "success";
      g_response["responsedata"] = "Topics Updated Successfully";
      g_status_code = 201;
    } catch (err) {
      g_response["status"] = "error";
      g_response["message"] = err.message;
...
exports.DeleteById = async (req, res) => {
  ...
  if (req.params.id && req.params.id.length > 0) {
    ...
    try {
      const delete_resp = await topicsDelete(deleteID);
      g_response["status"] = "success";
...
const topicsDelete = async (deleteID, result) => {
  let delete_payload = {
    table_name: table,
    query_field: "id",
