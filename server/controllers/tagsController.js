...
exports.createtags = async (req, res) => {
  ...
  try {
    ...
    try {
      const create_data = await SQL.addData("tags", data);
...
exports.gettagsByID = async (req, res) => {
  ...
  if (req.params.id && req.params.id.length > 0) {
    ...
    try {
      const get_tags = await SQL.gettabledata("tags", [], { id: ID });
      g_response["status"] = "success";
...
exports.getAllTags = async (req, res) => {
  ...
  try {
    const get_tags = await SQL.gettabledata("tags", []);
    g_response["status"] = "success";
    g_response["responsedata"] = get_tags;
    g_status_code = 200;
  } catch (err) {
...
exports.updatetagsByID = async (req, res) => {
  var ID;
  console.log(req.body);
  var data = req.body;
  var g_response = {};
  ...
  if (req.params.id && req.params.id.length > 0) {
    ...
    try {
      const valid_data = await validation(data, validationRule);
      const update_data = await SQL.Updatedata("tags", data, {
        id: ID,
      });

      g_response["status"] = "success";
      g_response["responsedata"] = "Tags Updated Successfully";
      g_status_code = 201;
    } catch (err) {
      g_response["status"] = "error";
      g_response["message"] = err.message;
...
exports.DeleteById = async (req, res) => {
  ...
  if (req.params.id && req.params.id.length > 0) {
    ...
    try {
      const delete_resp = await tagsDelete(deleteID);
      g_response["status"] = "success";
      g_response["message"] = "Tags Delete Successfully";
      g_status_code = 201;
    } catch (err) {
      g_response["status"] = "error";
      g_response["message"] = err.message;
...
const tagsDelete = async (deleteID, result) => {
  let delete_payload = {
    table_name: table,
    query_field: "id",
