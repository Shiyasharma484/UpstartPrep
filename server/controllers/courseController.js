...
exports.createCourse = async (req, res) => {
  ...
  try {
    ...
    const get_title = await SQL.gettabledata(" course", "id", {
      title: check_title,
    });
    if (get_title[0]) {
      ...
      const create_data = await SQL.addData("course", data);
      const get_Add_data = await SQL.gettabledata("course", [], {
        id: create_data,
      });
      g_response["status"] = "success";
      g_response["message"] = `Course Created  Successfully`;
...
exports.getCourseByID = async (req, res) => {
  ...
  if (req.params.id && req.params.id.length > 0) {
    ID = req.params.id;
    try {
      var get_data = await SQL.gettabledata("course", ["id"], { id: ID });
      if (get_data.length < 1) {
        ...
        var get_data = await SQL.query(
          "SELECT c.id,c.title,c.overview,c.topics,t.topic_name,c.skills,s.skill_name,c.attachment,c.contents,co.title as content_title FROM course as c LEFT JOIN skills as s ON s.id=c.skills LEFT JOIN topics as t ON t.id=c.topics LEFT JOIN contents as co ON co.id=c.contents WHERE c.id=" +
            ID
        );
...
exports.getAllCourse = async (req, res) => {
  ...
  try {
    var get_data = await SQL.query(
      "SELECT c.id,c.title,c.overview,c.topics,t.topic_name,c.skills,s.skill_name,c.attachment,c.contents,co.title as content_title FROM course as c LEFT JOIN skills as s ON s.id=c.skills LEFT JOIN topics as t ON t.id=c.topics LEFT JOIN contents as co ON co.id=c.contents "
    );
