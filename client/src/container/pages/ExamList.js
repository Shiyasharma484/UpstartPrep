  ...
  useEffect(() => {
    ...
    if (enc_user_detail) {
      ...
      if (RoleName == 'instructor') {
        GetExamsByInstructorID(deodedID);
      } else {
        GetAllExams();
      }
    }
    ...
    async function GetExamsByInstructorID(id) {
      const url = api_url.getExam_byInstructorID + id;
      console.log(url);
      const response = await get_api_request(url, headers);
      console.log(response);
      if (response.status == 200) {
    ...
    <>
      <Main className="ExamList" style={{ minHeight: 'auto' }}>
        <div key="1" className="page-header-actions" style={{ marginTop: '16px', justifyContent: 'space-between' }}>
          ...
          >
            <h1 style={{ fontSize: '25px', color: 'rgb(20 19 19)' }}>Exams</h1>
          </div>
            ...
            >
              <Button
                ...
                onClick={() =>
                  ...
                  history.push(`../engage/create-exam`)
                }
                size="small"
                key="4"
                type="success"
