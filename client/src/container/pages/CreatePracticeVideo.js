  ...
  const addDetails = fieldsvalue => {
    ...
    if (Practice) {
      async function updateLesson() {
        const url = api_url.get_video_bank_id + PracticeId;
      ...
      async function CreateSection(data) {
        ...
        if (response.status == 201) {
          ...
          async function getQuestionbyid() {
            const url = api_url.get_video_bank_id + responseid;
            const response = await get_api_request(url, headers);
            if (response.status == 200) {
            ...
            <Cards>
              <div className="SectionTabs_Main">
                <div id="SectionTabsGeneral" className="SectionTabsInner">
                  <p>
                    <h2 className="titlepractice">Configure Videos</h2>
                    <p>Add Videos to Learning</p>
                  </p>
