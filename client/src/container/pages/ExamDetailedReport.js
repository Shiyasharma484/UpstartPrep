...
const ExamDetailedReport = () => {
  ...
  const HandleAction = e => {
    ...
    const id = e;
    if (e == 3) {
      history.push(`../users/${id}/edit-exam`);
    } else if (e == 2) {
      history.push(`../users/${id}/edit-exam`);
    } else if (e == 1) {
      history.push(`../users/${id}/overallExam-report`);
    }
  };
      ...
      <Main className="Quiz_report">
        <div key="1" className="page-header-actions" style={{ marginTop: '16px', justifyContent: 'space-between' }}>
          ...
          <div className="importNewBTN">
            ...
            <Button onClick={() => history.push(`./overallExam-report`)} size="small" key="4" type="success">
              <FeatherIcon icon="plus" size={14} />
              OVERALL REPORT
            </Button>
          </div>
