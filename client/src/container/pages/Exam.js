...
const Exam = () => {
  ...
  useEffect(() => {
    ...
    const student_id = response?.sessdata?.users_id_enc;
    setUserID(userId);
    //
    const question_data = Cookies.get(userId + 'question');
    const time_data = Cookies.get(userId + 'data');
    ...
    async function GetQuestionByExamID() {
      const url = api_url?.get_allquestion_byexamID + EventId;
      var request = { schedule_id: ScheduleId, student_id: student_id };

      const response = await post_api_request(url, request, headers);
      console.log(response);
      if (response?.status == 200 && response?.data?.message == 'Exam Data') {
        const SettingMeta = response?.data?.meta?.settings;
        const suffleQuestion = response?.data?.meta?.settings?.suffleQuestionAuto; //
        const time_duration = response?.data?.duration ? Number(response?.data?.duration) : 120;
        const time_inSeconds = time_duration * 60;
        // const all_questions = response?.data?.data;
      ...
      } else if (response?.data?.message == 'Already Attempted') {
  ...
  const formatTime = time => {
    ...
    Cookies.set(userid + 'data', StringFinalData);
  ...
  const SetCookies = () => {
    ...
    //
    console.log(FinalQuestionArray);
    console.log(StringFinalData);
    Cookies.set(userid + 'question', StringFinalQuestionArray);
    Cookies.set(userid + 'data', StringFinalData);
  };
  /**SET COOKIES==============================================================ENDS **/
  ...
  const HandleExam = () => {
    ...
    async function SubmitExam() {
      const url = api_url.exam_taken;
      console.log(headers);
      const response = await post_api_request(url, stringifyData, headers);

      console.log(response);
      if (response.status == 201) {
        const marks_obtained_id = response?.data?.marks_obtained_id;
        const encryptedID = encrypttheid(marks_obtained_id);
        setIsBilling(false);
        notification.success({
          message: 'Exam Submitted Successfully!',
        });
        Cookies.remove(userid + 'question');
        Cookies.remove(userid + 'data');
        history.push({
          pathname: '/exam-success',
          state: encryptedID ? encryptedID : 'No Data',
        });
        //
        // history.push('/exam-success');
      } else {
        notification.error({
                  ...
                  <div
                    className={
                      QuestionIndex == index
                        ...
                        ? 'inner-qdiv-comebacklater left-number'
          ...
          <Spin spinning={loading}>
            <Row className="quizQRow">
              <Col md={17} style={{ paddingLeft: '5px', paddingRight: '5px' }}>
                <Row>
                  <Col md={4} className="QuestioniBTN" style={{ paddingLeft: '5px', paddingRight: '5px' }}>
                  ...
                  {/* {console.log(SingleQuestion, '555555555555555555')} */}
                  <Col md={20} className="qusetionp" style={{ paddingLeft: '5px', paddingRight: '5px' }}>
            ...
            <Row className="nextPrevbtn">
              ...
              <Col md={12}>
                <div className="quest_Btns_pre_nxt justiRight">
                  <Button
                    className="nextbtn"
                    onClick={() =>
          ...
          <div className="payment-success-main exam-success">
            <div className="payment-success-inner">
              ...
              <div className="payment-text">
                <p className="payment-sucess">Already Attempted!</p>
              </div>
            </div>
          </div>
        </Main>
