...
const CreateNewQuiz = () => {
  ...
  useEffect(() => {
    ...
    async function GetUserGroupDataByID() {
      console.log(id);
      const url = api_url.get_lesson_id + id;
      const response = await get_api_request(url, headers);
  ...
  const addNewUser = fieldsvalue => {
    ...
    payload['tag_id'] = settingData.tag_id;
    ...
    async function CreateLesson(data) {
      const url = api_url.update_lesson_id + formData?.id;
                    ...
                    <Col md={24} xs={24} className="mb-space">
                      <Form.Item name="tag_id" label="Tags">
                        <Select
                          ...
                          //onSelect={GetPosition}
                          onChange={selectedValue => {
                            setsettingData({ ...settingData, tag_id: selectedValue });
                          }}
                        >
                          {arrayDatatag != null
