...
const CreateNewExam = () => {
  ...
  useEffect(() => {
    ...
    async function GetAllSchedules() {
      const url = api_url.get_schedules_by_event_id + QuizID + '/' + '30';
      const response = await get_api_request(url, headers);
      console.log(response);
      if (response.status == 200) {
    ...
    async function GetAllSectionData() {
      const url = api_url.get_section_by_exam_id + QuizID;
      const response = await get_api_request(url, headers);
    ...
    if (actionid == 3) {
      ...
      async function getsectionbyid() {
        ...
        if (sectionDatabyId?.active == 1) {
          setEditActiveStatusSwitch1(true);
        } else if (sectionDatabyId?.active == 0) {
          setEditActiveStatusSwitch1(false);
        }
        seteditsectionData({
          ...editsectionData,
  ...
  const AddQuizDetails = (fieldsvalue, name) => {
    console.log(SettingsData?.created_by);
    if (QuizzData) {
      var meta = {};
      var AddquizDetails = {
      ...
      meta = {
        ...
        showScoreBoardAuto: fieldsvalue?.showScoreBoardAuto,
        showScoreBoardManual: fieldsvalue?.showScoreBoardManual,
    ...
    if (QuizzData) {
      async function UpdateExam() {
        const url = api_url.update_exam_byId + QuizID;
  ...
  async function GetQuestionBySectionID(section_id) {
    ...
    // // const ExamID = id;
    // const url = api_url.getquestion_bysectionID + section_id;
    // const response = await get_api_request(url, headers);
    // console.log(response);
    // if (response.status == 200) {
  ...
  async function getaddedquestions(sectionID) {
    ...
    const url = api_url.getexam_question + QuizID;
    const response = await get_api_request(url, headers);
    if (response.status == 200) {
                  ...
                  >
                    <Row gutter={30} className="skillrow mb-space">
                      ...
                      {RoleName == 'super_admin' ? (
                        <Col md={18} xs={24} className="skillrow-inner mBottom" id="create_by">
                          <Form.Item
                            name="created_by"
                            label="Created By"
                            rules={[
                      ...
                      <Col md={18} xs={24} className="skillrow-inner mBottom" id="category">
                        <Form.Item
                          name="category_id"
                          label="Category"
                          rules={[
                      ...
                      <Col md={18} xs={24} className="skillrow-inner mBottom" id="exam_type">
                        <Form.Item
                          name="exam_type"
                          label="Exam Type"
                          rules={[
                      ...
                      {showsolutionvalue == true ? (
                        <>
                          <Row guttor={30}>
                            <Col md={18} className="skillrow-inner mBottom">
                              <Form.Item
                                label="Price"
                                name="price"
                                rules={[
                      ...
                      <Col md={12} className="First_Division">
                        ...
                        <Col md={24}>
                          ...
                          >
                            <Input
                              placeholder="60%"
                              defaultValue={SettingsData?.pass_percentage}
                              onChange={selectedValue => {
                        ...
                        {console.log(SettingsData?.terms_condition)}
                          ...
                          <Col className="AddQuizTpe" md={24} xs={24}>
                            <Row gutter={30}>
                              <Col md={24} xs={24}>
                                ...
                                <Form.Item name="schedule_type" initialValue={settingData?.flex}>
                                  <input
                                    type="radio"
                                    id="html"
                                    name="fav_language"
                                  ...
                                  />
                                  {console.log(settingData?.flex)} <label for="html">Fixed</label> {' '}
                                  <input
                                    type="radio"
                                    id="css"
                                    name="fav_language"
                        ...
                        <Row gutter={30} className="AddQuizTpe_main" style={{ marginTop: ' 10px' }}>
                          <Col className="AddQuizTpe" md={24} xs={24}>
                            <Row gutter={30}>
                              ...
                              <Col md={24} xs={24}>
                                <Form.Item name="schedule_to_group" label="Schedule to User Groups">
                                  <Select
                                    mode="multiple"
                                    allowClear
                                    showSearch
